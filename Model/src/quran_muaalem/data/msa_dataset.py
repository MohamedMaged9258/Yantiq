...
class MSAPhonemeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feat = features["input_features"]  # (1, T, 160)
        mask = features["attention_mask"]  # (1, T)
        T = feat.shape[1]
        if T > self.max_features:
            features["input_features"] = feat[:, :self.max_features, :]
            features["attention_mask"] = mask[:, :self.max_features]
        elif T < self.max_features:
            pad = self.max_features - T
            features["input_features"] = torch.nn.functional.pad(feat, (0, 0, 0, pad))
            features["attention_mask"] = torch.nn.functional.pad(mask, (0, pad))
