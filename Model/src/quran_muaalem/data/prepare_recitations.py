...
full ~97.8 GB at once) and materialized as flat WAV files under `<output_dir>/audio/`.
...
class RecitationsProcessor:
    ...
    def process(
        ...
        for config in configs:
            ...
            kept_here = 0
            for row in tqdm(ds, desc=f"  {config}"):
    ...
    def _process_row(self, row: dict, config: str, idx: int) -> Optional[dict]:
        ...
        try:
            ...
            out_path = self.audio_dir / f"{safe_config}_{idx:06d}.wav"
